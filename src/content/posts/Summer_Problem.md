...
description: "求树上多少条简单路径，其点集满足 刚好为 - 2025.7.18 航电第一场 1007 - 我们定义以下函数： - 我们可以写出一下式子： - 那么就是最基础的莫比乌斯反演，得到 - 统计 是简单的，并查集然后统计连通…"
tags: ["DS"]
category: "竞赛题解"
legacySlug: "Summer_Problem"
draft: false
---
$$\text{夏の終わり}$$
...
\begin{cases}
dp_{i,1}=\min\left\{
dp_{i-1,1}+\max\{0,c_{i,1}-c_{i-1,1}\},
dp_{i-1,0}+\max\{0,c_{i,1}-c_{i-1,0}\}
\right\} \\
dp_{i,0}=\min\left\{
dp_{i-1,1}+\max\{0,c_{i,0}-c_{i-1,1}\},
dp_{i-1,0}+\max\{0,c_{i,0}-c_{i-1,0}\}
\right\}
\end{cases}
$$
